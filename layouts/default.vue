<template>
  ...
  <a-drawer v-model:visible="showDrawer" placement="left" :closable="false" width="300">
    ...
    <li v-for="(i, index) in urls" :key="index" @click="showDrawer = false"
      ...
      <div
        class="absolute left-0 bottom-4 w-full h-1 rounded-t-md bg-gray-300 -z-10 group-hover:h-1/3 group-hover:transition-all dark:bg-white dark:z-0 dark:bg-opacity-50">
      </div>
    </li>
